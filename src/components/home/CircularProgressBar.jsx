...
const CircularProgressBar = ({ percentage, width, height, percentageSize }) => {
  ...
  return (
    <div
      className={`relative flex items-center justify-center ${width ? width : "w-[95px]"} ${
    ...
    >
      ...
      <span
        className={`absolute ${percentageSize ? percentageSize : "text-[18px]"} font-semibold text-[#000]`}
      >{`${percentage}%`}</span>
    </div>
  );
};
