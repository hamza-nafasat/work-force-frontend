...
const ProjectCard = ({ title, images, startDate, endDate, percentage, progressColor }) => {
  ...
  return (
    <>
      <div
        className="bg-white px-4 md:px-6 xl:px-8 pt-4 md:pt-[20px] pb-4 md:pb-6 xl:pb-8 shadow-md rounded-[12px] border-l-[12px] mb-4"
        style={{ borderColor: `${borderColor}` }}
      >
        <h4 className="text-base md:text-[20px] text-[#000] font-semibold">{title}</h4>
          ...
          <div>
            ...
            <div className="flex flex-wrap items-center gap-4 justify-between mt-4">
              ...
              <div className="flex flex-col">
                ...
                <p className="text-sm md:text-[18px] font-medium md:font-semibold text-[#000]">{endDate}</p>
              </div>
            </div>
          </div>
          <div className="flex flex-col items-center justify-items-center gap-1">
            <CircularProgressBar percentage={Number(percentage)} color={progressColor} />
